...
from pytorch_OpCounter import thop
import torch
...
if __name__ == "__main__":
    ...
    if not torch.cuda.is_available():
        assert 1>3
    net = DeRain_v2().to(device)
    inputs = torch.randn(1, 3, 32, 32).cuda()
    print(net(inputs)[0].shape, net(inputs)[1].shape)

    macs, params = profile(net, inputs=(inputs, ))
    macs, params = clever_format([macs, params], "%.3f")
    print(macs, params) # 326.951M 2.606M