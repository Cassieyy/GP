...
def kernel_linear(X_u,X_l,vector_length,Num_A,Num_B): # linear kernel
    
    x_l_t = X_l.repeat(Num_A,1)
    x_u_t = X_u.repeat(1,Num_B)
    x_u_t = x_u_t.view(Num_A*Num_B,vector_length)
    ker_t = torch.nn.functional.cosine_similarity(x_u_t,x_l_t) # 计算两个vector的余弦相似度
    # ker_t = torch.nn.Parameter(
    #         nn.init.xavier_uniform_(torch.FloatTensor(Num_A, Num_B)), requires_grad=True
    #         )
    # print(ker_t.shape) # 计算相似度的两个换掉 # torch.Size([1024])
    ker_t = ker_t.view(Num_A,Num_B) # torch.Size([32, 32])
    # assert 1>3
    return ker_t
...
class GPStruct(object):
    def __init__(self,num_lbl,num_unlbl,train_batch_size,version,kernel_type):
        ...
        # 用一个可学习的矩阵 or se attention
        # self.ker_lbl = torch.nn.Parameter(
        #     nn.init.xavier_uniform_(torch.FloatTensor(self.num_lbl,self.num_lbl)), requires_grad=True
        #     )
            
        self.ker_unlbl = np.zeros((self.num_unlbl,self.num_lbl)) # kernel matrix of unlabeled vectors
        # self.ker_unlbl = torch.nn.Parameter(
        #     nn.init.xavier_uniform_(torch.FloatTensor(self.num_unlbl,self.num_unlbl)), requires_grad=True
        #     )
    ...
    def gen_featmaps(self,dataloader,net,device):
        ...
        for batch_id, train_data in enumerate(dataloader):
            ...
            for i in range(tensor_mat.shape[0]):
                ...
                tensor = torch.squeeze(tensor_mat[i,:,:,:]) # 32 32 32 
                # print(tensor.shape)
    ...
    def compute_gploss(self,zy_in,imgid,batch_id,device,label_flg=0):
        tensor_mat = zy_in
        
        Sg_Pred = torch.zeros([self.train_batch_size,1])
        Sg_Pred = Sg_Pred.to(device)
        LSg_Pred = torch.zeros([self.train_batch_size,1])
        LSg_Pred = LSg_Pred.to(device)
        gp_loss = 0
        ...
        for i in range(tensor_mat.shape[0]):
            ...
            if self.kernel_type =='Linear':
                ...
                # print("ker_LL.shape:", ker_LL.shape) # 256 256
                # assert 1>3
            else :
            ...
            inv_ker = inv_ker.to(device)
            farinv_ker = farinv_ker.to(device)
            near_vec_lbl = near_vec_lbl.to(device)
            far_vec_lbl = far_vec_lbl.to(device)

            mn_pre = torch.from_numpy(mn_pre) # used for mean prediction (mu) or z_pseudo 
            mn_pre = mn_pre.to(device)
            ...
            if self.version == 'version1':
                ...
                if self.kernel_type =='Linear':
                    ...
                    ker_UL = self.kernel_comp(tensor_vec,near_vec_lbl,self.var_Fz_lbl)
                sigma_est = ker_UU.to(device) - torch.matmul(ker_UL.to(device),torch.matmul(inv_ker.to(device),ker_UL.t().to(device))).to(device) + Eye
                # computing variance between farthest labeled vectors and unlabeled vector
                if self.kernel_type =='Linear':
                    ...
                    Farker_UL = self.kernel_comp(tensor_vec,far_vec_lbl,self.var_Fz_lbl)
                far_sigma_est = ker_UU - torch.matmul(Farker_UL.to(device),torch.matmul(farinv_ker.to(device),Farker_UL.t().to(device))) + Eye
            
            # computing mean prediction
            mean_pred = torch.matmul(ker_UL.to(device),mn_pre.to(device)) #mean prediction (mu) or z_pseudo
        ...
        if not (batch_id % 100) and loss_unsup==loss_unsup:
            print("LSg_Pred:", LSg_Pred.max().item(), "Sg_Pred:", Sg_Pred.max().item(), "gp_loss:", gp_loss.item()/self.train_batch_size, "Sg_Pred.mean():", Sg_Pred.mean().item())
        return gp_loss
